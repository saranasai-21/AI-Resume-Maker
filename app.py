...
st.set_page_config(
    ...
    page_icon="🚀",
    layout="centered",  # Standard centered single column layout for maximum readability
...
st.markdown("""
<style>
/* Curated gradient and premium vertical spacing */
.main .block-container {
    padding-top: 2rem;
    padding-bottom: 3rem;
    max-width: 860px; /* Center container with ideal width for A4 sheets and inputs */
}
...
st.markdown('<div class="card-title">📁 1. Upload & Settings</div>', unsafe_allow_html=True)
...
# Section B: Target Job Requirements
...
with tab_role:
    ...
    st.info("💡 If a full job description is unavailable, the AI will research and tailor the resume based on standard industry expectations for this role.")
...
# Section C: Advanced Directives
st.markdown("##### **Company Specific Wording (Optional)**")
...
# --- RESULTS WORKSPACE ---
st.markdown("---")
st.markdown("### 👁️ Tailored Resume Output")
...
if optimize_btn:
    if uploaded_resume is None:
        ...
        st.error("⚠️ Please enter the company name or disable Target Company Alignment.")
    else:
...
if st.session_state['optimized_html']:
    ...
    # Card 2: Output Container
    st.markdown('<div class="custom-card">', unsafe_allow_html=True)
    
    # 1. Download Options Card
    st.markdown(f"""
    <div class="download-card">
        <div class="download-title">✅ Optimization Complete!</div>
        <div style="font-size: 13.5px; color: #1e293b; margin-bottom: 15px;">
            Your resume was optimized using <strong>{st.session_state['active_provider']}</strong> and formatted to fit exactly on a single A4 page. Use the download buttons below to save your files.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px; padding: 14px; margin-top: 15px; font-size: 12.5px; color: #1e3a8a; line-height: 1.5;">
        💡 <strong>How to Save as A4 PDF:</strong> Open the downloaded HTML file in Google Chrome (or any browser), press <code>Ctrl + P</code> (or <code>Cmd + P</code>), set Margins to <strong>'None'</strong>, enable <strong>Background Graphics</strong>, and save as PDF.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="skeleton-box">
        ...
        <p style="margin: 0 auto 25px auto; font-size: 14px; max-width: 450px; color: #6b7280; line-height: 1.5;">
